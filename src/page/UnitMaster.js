...
export default function UnitMaster() {
  ...
  const [alowed, setAlowed] = useState("");
    ...
    <div className="master-container">
      ...
      <div className="master-card">
        ...
        {edit.type === "unit" ? (
          ...
          {state.units.length===0 ?null : <button onClick={()=>setAlowed("linepage")}>Next</button>}
          </>
        )}
        <ul>
...
<select className="selectdrop" name="unit" value={unitbdrop} onChange={(e)=>setUnitbdrop(e.target.value)}>
        ...
        {edit.type === "line" ? (
          ...
          <>
         {alowed ==="linepage" || state.lines.length!==0 ?  <button onClick={() => handleAdd("line")}>Add Line</button>:null}

      {state.lines.length===0 ? null:   <button onClick={()=>setAlowed("shiftpage")} >Next</button>}
          </>
        )}
        <ul>
      ...
      <div className="master-card">
        ...
        {edit.type === "shift" ? (
          ...
          <>
          {alowed ==="shiftpage" || state.shifts.length!==0 ? <button onClick={() => handleAdd("shift")}>Add Shift</button>:null}
          </>
         )}
        <ul>
